...
Solved at: 2026-04-12 (30m)

Simulation

1. 한 붓 그리기 꼴이므로 현재 노드의 상하좌우에는 1가지 밖에 없다는 점을 이용.
   시작과 끝만 보면 된다.
...
현대자동차그룹에 입사하여 로봇 연구 개발 부서에 막 입사한 당신은 아래와 같은 기능을 하는 간단한 로봇의 사용법을 전달받았다. 로봇은 $N$행 $M$열의 2차원 격자판 위를 돌아다닌다. 격자판의 각 칸은 정사각형 모양이며, 로봇은 격자판의 칸 하나를 차지한다. 로봇은 오른쪽(동), 왼쪽(서), 아래(남), 위(북) 중 한 방향을 바라볼 수 있다.
...
첫 번째 줄에 격자판의 세로 크기 $N$와 가로 크기 $M$가 공백 하나를 사이로 두고 주어진다.

다음에는 사수가 넘겨준 지도가 주어진다. $N$개의 줄에 길이 $M$의 문자열이 주어지며, 이 중 $i$번째 줄의 $j$번째 문자는, 사수가 조작한 로봇이 $i$행 $j$열을 방문했다면 '#'이고, 방문하지 않았다면 '.'이다.
...
첫 번째 줄에 두 개의 정수 $(r, c)$를 공백 하나씩을 사이로 두고 출력한다. 이는 처음 로봇을 격자판의 $r$행 $c$열에 두어야 함을 의미한다.
